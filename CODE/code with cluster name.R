...
########New Figure Generator


# Assuming the Seurat object and previous steps are done and you have 'seurat_combined'

# Create a named vector for cluster labels based on the provided table
cluster_labels <- c("0" = "CD4+ T cells",
                    "1" = "T cells",
                    "2" = "T cells",
                    "3" = "CD4+ T cells",
                    "4" = "Basophils",
...
# Update the cluster identities in the Seurat object
seurat_combined <- RenameIdents(seurat_combined, cluster_labels)

# Plot the UMAP with the new labels
DimPlot(seurat_combined, reduction = "umap", label = TRUE, label.size = 5, pt.size = 0.5) + 
  ggtitle("UMAP Plot with Cluster Names") + 
  theme(plot.title = element_text(hjust = 0.5))

# Save the plot if needed
#ggsave("D:/Bioinformatics/Single Cell/HIV/UMAP_with_cluster_names.png")
...
VlnPlot(seurat_combined, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3, pt.size = 0) + 
  ggtitle("Violin Plots of Quality Control Metrics")
...
DoHeatmap(seurat_combined, features = top10$gene) + NoLegend()

# Save the plot
#ggsave("D:/Bioinformatics/Single Cell/HIV/Heatmap_Top10Genes.png")
...
all.genes <- unique(cluster.all.markers0.5$gene)
seurat_combined <- ScaleData(seurat_combined, features = all.genes)
...
# Ensure all genes are scaled
all.genes <- unique(top10$gene)
seurat_combined <- ScaleData(seurat_combined, features = all.genes)

# Generate the heatmap
heatmap <- DoHeatmap(seurat_combined, features = top10$gene, group.by = "ident", slot = "scale.data") + 
  theme(
    axis.text.y = element_text(size = 6),  # Adjust font size for better readability
    axis.text.x = element_text(angle = 45, hjust = 1, size = 8)  # Rotate x-axis labels for better fit
  ) + 
  NoLegend() + 
  ggtitle("Heatmap of Top 10 Marker Genes per Cluster")

# Save the plot
#ggsave("D:/Bioinformatics/Single Cell/HIV/Heatmap_Top10Genes_PerCluster.png", plot = heatmap, width = 10, height = 8, dpi = 300)
...
# Generate the heatmap
heatmap <- DoHeatmap(seurat_combined, features = top10$gene, group.by = "ident") + 
  scale_fill_viridis_c() +  # Use a color palette for better visibility
  theme(
...
# Save the plot
ggsave("D:/Bioinformatics/Single Cell/HIV/Heatmap_Top10Genes.png", plot = heatmap, width = 10, height = 8, dpi = 300)
